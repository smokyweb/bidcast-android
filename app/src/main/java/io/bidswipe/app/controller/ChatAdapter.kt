		...
		if (chat.type != Chats.ChatType.DATE) {
			if (chat.users?.senderId == senderId) {
				with(holder as SentViewHolder) {
					...
					bind.replyView.root.setBackgroundColor(ContextCompat.getColor(mCtx , clr.primaryContainer))
					...
					if (chat.replyMessage?.type == "image") {
						bind.replyView.replyImage.loadUrl(mCtx,chat.replyMessage?.message.toString())
					...
					if (chat.replyMessage?.senderName == (Prefs(mCtx).getUserData()?.firstName + " " + Prefs(mCtx).getUserData()?.lastName)) {
			...
			with(holder as DateViewHolder) {
				...
				try {
					...
					calendar.timeInMillis = Instant.ofEpochSecond(chat.timestamp ?: 0L).toEpochMilli()
		...
		return when (type) {
			ITEM_RECEIVE -> ReceiveViewHolder(
				ReceiverChatItemBinding.bind(
					LayoutInflater.from(parent.context).inflate(R.layout.receiver_chat_item , parent , false)
				)
			)

			ITEM_DATE -> DateViewHolder(
				DateChatItemBinding.bind(
					LayoutInflater.from(parent.context).inflate(R.layout.date_chat_item , parent , false)
				)
			)

			else -> SentViewHolder(
				SenderChatItemBinding.bind(
					LayoutInflater.from(parent.context).inflate(R.layout.sender_chat_item , parent , false)
				)
			)
		}
	}
