...
class ShowListingAdapter(
	...
	override fun onBind(
		...
		with(holder) {
			...
			bind.time.text = "${Utils.getFormattedDateTime("yyyy-mm-dd", "mm-dd-yyyy", item?.date.toString())} ${Const.BULLET} ${Utils.getFormattedDateTime("HH:mm:ss", "hh:mm a", item?.time.toString())}"
