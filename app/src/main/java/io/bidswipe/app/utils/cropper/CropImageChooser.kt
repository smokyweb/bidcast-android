...
internal class CropImageChooser(
	...
	fun showChooserIntent(
		...
		val target = if (allIntents.isEmpty()) {
			...
			Intent(Intent.ACTION_CHOOSER, MediaStore.Images.Media.EXTERNAL_CONTENT_URI).also {
				if (includeGallery) {
	 ...
	 */
	fun setupPriorityAppsList(appsList: List<String>): CropImageChooser = apply {
	 ...
	 */
	fun setIntentChooserTitle(title: String): CropImageChooser = apply {
		this.title = title
	}
}