...
class Chats(private val mCtx : Context , private val chatKey : String , private val users : ChatModel.Users) {
	private var progressBind = ProgressAlertViewBinding.bind(LayoutInflater.from(mCtx).inflate(layout.progress_alert_view , null , false))
...
*/
	...
	private fun checkIfSameDay(check : (day : Pair<Long , String>) -> Unit) {
		FireRef.CHAT.child(chatKey).orderByKey().limitToLast(1).addListenerForSingleValueEvent(object : ValueEventListener {
			...
			override fun onDataChange(snapshot : DataSnapshot) {
				...
				val messageDate = format.format(Instant.ofEpochSecond(previousTime).toEpochMilli())
...
*/
	...
	private fun addToChatList(model : ChatModel) {
		model.id = chatKey
		FireRef.CHAT_LIST.child(users.receiverId.toString()).child(users.senderId.toString()).setValue(model.toMap())
		FireRef.CHAT_LIST.child(users.senderId.toString()).child(users.receiverId.toString()).setValue(model.toMap())
	}

	fun updateChatList(map : Map<String , Boolean>) {
		FireRef.CHAT_LIST.child(users.receiverId.toString()).child(users.senderId.toString()).updateChildren(map)
	}
}