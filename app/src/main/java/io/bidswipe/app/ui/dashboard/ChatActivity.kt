...
class ChatActivity : BaseActivity() {
    ...
    private val mClick = object : RecyclerClicks {
        override fun itemClick(pos: Int, status: String?) {
            val item = chatList[pos]
            when (status) {
                "reply_click" -> {
                    val notifyIndex =
                        chatList.indexOf(chatList.find { it.id == item.replyMessage?.messageId })
                ...
                "root", "image" -> {
                    ...
                    if (item.type == Chats.ChatType.SHARE) {
                        ...
                        val sellerId = message.split("seller/")
                        if (sellerId.size > 1) {
                            val sellerIdValue = sellerId.lastOrNull() ?: ""
                            if (sellerIdValue.isNotEmpty()) {
                                startActivity(
                                    Intent(this@ChatActivity, SellerProfileActivity::class.java).putExtra(
                        ...
                        val productId = message.split("products/")
                        if (productId.size > 1) {
                            val productIdValue = productId.lastOrNull() ?: ""
                            if (productIdValue.isNotEmpty()) {
                                startActivity(
                                    Intent(this@ChatActivity, ProductDetailsActivity::class.java).putExtra(
                        ...
                        val roomID = message.split("roomId=")
                        if (roomID.size > 1) {
                            ...
                            if (splitted.size > 1) {
                                ...
                                if (userRoom?.size == 2) {
                                    ...
                                    if (userId.isNotEmpty() && roomId.isNotEmpty()) {
                                        startActivity(
                                            Intent(this@ChatActivity, ViewLiveShowActivity::class.java)
