...
class DashViewModel @Inject constructor(
    ...
    fun storeProductMeta(productImages: List<MultipartBody.Part>?, videos: List<MultipartBody.Part>?, thumbnails: List<MultipartBody.Part?>?) =
        viewModelScope.launch {
            if (!networkMonitor.hasInternet()) {
    ...
    // Basecamp #9938023997: added category_ids + sub_category_ids filter pass-through
    ...
    fun getExploreLiveShow(
        ...
        _getExploreLiveShowResponse.value = repo.getExploreLiveShow(type, category, subCategory, search, page, showFormat, tag, premierShop, shipCountry, shipState, shipping, categoryIds, subCategoryIds)
    }
    ...
    private var _storeSurpriseProductResponse = MutableLiveData<Resource<CommonResponse>>()
    val storeSurpriseProductRepo: MutableLiveData<Resource<CommonResponse>>
        get() = _storeSurpriseProductResponse

    fun storeSurpriseProduct(
        request: StoreSurpriseSet?
    ) = viewModelScope.launch {
        if (!networkMonitor.hasInternet()) {
            _storeSurpriseProductResponse.value = NO_INTERNET_ERROR
            return@launch
        }
