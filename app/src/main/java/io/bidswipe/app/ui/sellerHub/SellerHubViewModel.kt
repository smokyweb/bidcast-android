...
class SellerHubViewModel @Inject constructor(
    ...
    fun storeShippingProfile(
        ...
        maxItemUnit: RequestBody? = null,
        height: RequestBody? = null,
        width: RequestBody? = null,
        length: RequestBody? = null,
        scale: RequestBody? = null,
        incrementWeight: RequestBody? = null,
        incrementWeightUnit: RequestBody? = null,
    ) = viewModelScope.launch {
        if (!networkMonitor.hasInternet()) {
            _storeShippingProfileResponse.value = NO_INTERNET_ERROR
            return@launch
        }
        _storeShippingProfileResponse.value = repo.storeShippingProfile(
            shippingProfileId,
            name,
            size,
            weight,
    ...
    private var _getUSPSBoxDimensionsResponse = MutableLiveData<Resource<GetUSPSboxDimensionsResponse>>()
    val getUSPSBoxDimensionsRepo: MutableLiveData<Resource<GetUSPSboxDimensionsResponse>>
        get() = _getUSPSBoxDimensionsResponse

    fun getUSPSBoxDimensions() = viewModelScope.launch {
        if (!networkMonitor.hasInternet()) {
            _getUSPSBoxDimensionsResponse.value = NO_INTERNET_ERROR
            return@launch
        }
        _getUSPSBoxDimensionsResponse.value = repo.getUSPSBoxDimensions()
    }

    private var _saveDomesticShipmentSettingResponse = MutableLiveData<Resource<CommonResponse>>()
    val saveDomesticShipmentSettingRepo: MutableLiveData<Resource<CommonResponse>>
        get() = _saveDomesticShipmentSettingResponse

    fun saveDomesticShipmentSetting(
        ...
        if (!networkMonitor.hasInternet()) {
            _saveDomesticShipmentSettingResponse.value = NO_INTERNET_ERROR
            return@launch
        }
        _saveDomesticShipmentSettingResponse.value =
